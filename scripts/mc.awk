...
BEGIN {
  ...
  printf("\"Tran Date\",\"Post Date\",\"Reference Number\"," \
         "\"Description of Transaction or Credit\",\"Amount\"\n")
}
# awk '/^Tran Date/,/^\(Continued on next page\)/ || /^FEES/' 20111025.txt
#/^Tran Date/,/^\(Continued on next page\)/ || /^FEES/
/^Tran Date/ {
  if (is_tran == 0) {
    is_tran = !is_tran
  }
...
is_tran == 1 {
  ...
  if ((dollar_seek == 0) &&
      ($1 ~ /[0-9][0-9]\/[0-9][0-9]/) &&
      # NOTE: comment out the line below and uncomment the two lines following to recognize the
      # third token as well.  For this POSIXLY_CORRECT needs to be defined, so run the script like:
      #
      # $ POSIXLY_CORRECT=1 mc.awk 20110525.txt
      #
      ($2 ~ /[0-9][0-9]\/[0-9][0-9]/)) {
      #($2 ~ /[0-9][0-9]\/[0-9][0-9]/) &&
      #($3 ~ /[[:alnum:]]{17}/)) {
