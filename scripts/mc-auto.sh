...
script="./mc.awk"
ppedir="../PayPal-Extras-MasterCard/"
#fordir=${ppedir}"formatted/"
fordir=${ppedir}

# $ ./mc.awk ../PayPal-Extras-MasterCard/20110222.txt > ../PayPal-Extras-MasterCard/formatted/20110222.csv
...
for file in ${ppedir}*.txt; do
  ...
  if [ ${force} -eq 1 ] || ! [ -f ${fordir}${bn}.csv ]; then
    newcommand=`echo "${script} ${file} > ${fordir}${bn}.csv; "`
    if [ ${doecho} -eq 1 ]; then
      echo ${newcommand}
    fi
